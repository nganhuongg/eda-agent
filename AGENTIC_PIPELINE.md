...
**Why is this the integration phase?**
Phases 1–4 were all built and tested in isolation. Phase 1 defined shared memory. Phase 2 built the Critic. Phase 3 built the loop. Phase 4 built the Analyst. None of them knew about each other — each module has no import from the others. Phase 5 is the moment of assembly: it wires all four components into a single coordinated loop running inside the orchestrator. The risk is highest here because the orchestrator is the central coordination point, and getting the wiring wrong affects every column in every run.

---

### Step 5.1 — From Step-Based to Column-Based Loop

**What the v2 orchestrator loop looks like:**

```python
# v2: one (column, action) pair per step
for step in range(1, max_steps + 1):
    state["signals"] = extract_signals(df, state["dataset_metadata"])
    state["risk_scores"] = compute_risk_scores(...)

    plan = risk_driven_planner(state)  # returns {"column": ..., "action": ...}
    if plan is None:
        return {"status": "SUCCESS", ...}

    column = plan["column"]
    action_name = plan["action"]
    action_result = ACTION_TO_TOOL[action_name](df, column, ...)
...
**What the v3 orchestrator loop looks like:**

```python
# v3: one full investigation per column
for _col_idx in range(max_columns):
    state["signals"] = extract_signals(df, state["dataset_metadata"])
    state["risk_scores"] = compute_risk_scores(...)

    plan = risk_driven_planner(state)  # still used for column selection
    if plan is None:
        return {"status": "SUCCESS", ...}

    column = plan["column"]

    # Tools first: populate analysis_results before the Analyst sees anything
    _run_tools_for_column(state, df, column, col_type, _col_idx)

    # Gate 1: Analyst+Critic loop (Ralph Loop)
    ...
    state["insights"][column] = _to_insights_bridge(decision)
    state["analyzed_columns"].add(column)
```

**Why switch from step-based to column-based?**

The v2 step-based loop was designed around a single action per step: pick a (column, action) pair, run it, generate an insight, move on. This made sense when insights were generated by a deterministic function — one tool call produces one insight. But in v3, each column requires a multi-turn conversation with the Analyst and Critic before a finding is approved. That conversation cannot happen in one step — it requires a loop within the loop.

The column-based structure makes this natural: the outer loop picks one column per iteration, and everything inside that iteration — tool execution, Analyst call, Critic validation, potential Analyst retry — is contained within a single column's investigation. The boundary is clean: the outer loop says "which column next?", and the inner logic says "what does this column mean?".

**Why keep `risk_driven_planner` for column selection?**

The v2 planner uses risk scores (missingness, skewness, outlier ratio) to rank columns and return the highest-risk unanalyzed one. That logic is correct and already tested. Phase 5 is about what happens *inside* each column's investigation, not about which column to pick next. Reusing the proven planner for selection means this phase only introduces one new thing at a time.

The planner's `plan["action"]` field (which previously specified which tool to run) is now ignored — tools are run deterministically in `_run_tools_for_column` rather than being delegated to the planner one at a time.

---

### Step 5.2 — Tools-First Execution: `_run_tools_for_column()`

**What happens:** Before the Analyst+Critic loop runs, all diagnostic analysis tools are executed for the target column:
...
def _run_tools_for_column(
    ...
    """Run analysis tools for a column. This is the ONLY place df is used in the loop."""
    tools_to_run = ["analyze_distribution", "detect_outliers"]
    if column_type == "categorical":
        tools_to_run = ["analyze_missing_pattern"]
    elif column_type == "numeric":
        tools_to_run = ["analyze_distribution", "detect_outliers", "analyze_missing_pattern"]

    state["analysis_results"].setdefault(column, {})
    for tool_name in tools_to_run:
        result = ACTION_TO_TOOL[tool_name](df, column, column_type) \
            if tool_name == "analyze_distribution" \
            else ACTION_TO_TOOL[tool_name](df, column)
        state["analysis_results"][column][tool_name] = result
        _record_action(state, step, f"tool:{tool_name}", None, "completed", result)
```

**Why run tools before the Analyst rather than after?**

There are two reasonable orderings:

**Option A (tools first):** Run all tools → populate `analysis_results` → then run Analyst+Critic loop. The Critic can validate claims against both `signals` AND `analysis_results`. Richer grounding.

**Option B (Analyst first):** Run Analyst → get `recommended_tools` → run those specific tools → then run Critic. The Analyst's output drives which tools execute.

Option B sounds more "agentic" — the LLM is deciding what to investigate. But it creates a problem: the Critic (`validate_finding`) validates claims by looking up field names in `signals` and `analysis_results`. If tools haven't run yet when the Critic first runs, the `analysis_results` dict is empty. Any claim that references a tool-output field (like a distribution percentile) would be rejected as unverifiable — even if the claim is correct. The Analyst would retry, tools still haven't run, the Critic still rejects. You'd exhaust 5 iterations without ever producing a valid result.

Tools-first solves this cleanly: by the time the Analyst produces claims and the Critic validates them, both data sources are available. The Analyst can reference signal fields (from Phase 1) or tool fields (just computed) with equal confidence.

**Why is `_run_tools_for_column` a separate function?**

It's the **df boundary**. This function is the only place in the v3 loop where `df` is used. Everything outside it — the Analyst call, the Critic validation, the loop control — operates on `state` only. By isolating all `df` access into one named function, you make the boundary:

1. **Explicit** — readers can immediately see where the raw data stops and the derived signals begin
2. **Testable** — a single mock of `_run_tools_for_column` in a test can eliminate all DataFrame access
3. **Enforceable** — a sentinel test (see Step 5.6) can verify that `analyze_column` never receives `df`

The alternative — inline tool calls scattered through `run_agent` — would make the boundary invisible and untestable.

---

### Step 5.3 — Gate 1: Wiring `run_loop` with `_make_gate1_critic`

**What happens:** The Analyst+Critic loop for each column is wired as follows:
...
def _make_gate1_critic(state: AgentState, column: str):
    """Return a critic_fn closure for Gate 1. Validates AnalystDecision claims."""
...
**Why `functools.partial` for the generator?**

`run_loop` was designed in Phase 3 to accept a `generator_fn` with the signature `(rejected_claims: List[str]) -> Any`. It calls `generator_fn(rejected_claims)` on each iteration. But `analyze_column` has the signature `(state, column, rejected_claims)` — it needs three arguments.

`functools.partial(analyze_column, state, column)` creates a new callable that has `state` and `column` pre-bound. When `run_loop` calls `generator_fn(rejected_claims)`, it becomes `analyze_column(state, column, rejected_claims)`. The loop doesn't need to know anything about `state` or `column` — it just calls its generator with the feedback from the previous iteration.

This is the key to keeping `run_loop` generic: it handles any `generator_fn` that takes a rejected_claims list, regardless of what else that function needs. Gate 2 (Phase 6) will use the same `run_loop` with a completely different generator, and `run_loop` won't need to change at all.

**Why a closure for `critic_fn` rather than a bound method?**

The Critic needs access to `state["signals"]` and `state["analysis_results"]` at the time it validates each `AnalystDecision`. These values change as the loop progresses (tools have already run, but in a multi-column scenario, state continues accumulating results). A closure captures `state` by reference — so `critic_fn` always sees the most current state when it validates. If it captured `state["signals"]` by value at creation time, it would be validating against stale data.

**Why `validate_finding` from Phase 2 rather than a new validation function?**

`validate_finding` is already tested, deterministic, and handles the exact claim format that `AnalystDecision.claims[]` produces. The Phase 2 design decision (two-source lookup: signals first, then analysis_results) was made precisely so this function could serve both the original critic use case and Gate 1 validation. Reusing it here is not laziness — it's the design working as intended.

---

### Step 5.4 — State Storage: `analyst_decisions` + `insights` Bridge

**What happens:** Each approved (or exhausted) `AnalystDecision` is stored in two places:

```python
# Primary storage: full AnalystDecision object
state["analyst_decisions"][column] = decision

# Bridge: backwards-compatible dict for v2 report consumers
...
`state["insights"]` is consumed by `report/report_generator.py`, `visualization/plot_generator.py`, and printed by `main.py`. These are the v2 report consumers that produce the final output. Changing what they expect would require rewriting the report generator in Phase 5 — which is Phase 6's job (the Global Synthesizer specifically replaces the report layer). Touching two things at once (orchestrator loop AND report format) doubles the regression risk.

The bridge lets Phase 5 focus entirely on the orchestrator loop. The report generator sees a `state["insights"]` dict in roughly the same shape it always has, and output still lands in `outputs/report.md` and `outputs/plots/`. Phase 6 then replaces the bridge with the synthesizer, which is a cleaner migration: one phase, one responsibility.

**Why add `analyst_decisions` as a new AgentState key rather than replacing `insights`?**

`AnalystDecision` is a Pydantic model. `AgentState["insights"]` is typed as `Dict[str, Dict[str, Any]]`. You could serialize `AnalystDecision` to a dict and store it in `insights`, but then any consumer reading `insights` would need to know which fields are new (hypothesis, recommended_tools) and which are old (summary, category). The result would be a mixed-format dict with unclear semantics.

A dedicated `analyst_decisions: Dict[str, Any]` key keeps the semantics clear: this key holds `AnalystDecision` objects (or their serialized equivalents), and everything in `insights` is the v2-compatible bridge format. Phase 6 reads from `analyst_decisions` for synthesis; v2 consumers read from `insights` for output. No ambiguity about which shape to expect.

**Why `Dict[str, Any]` instead of `Dict[str, AnalystDecision]` in the TypedDict?**

`AnalystDecision` lives in `agents/schemas.py`. `AgentState` lives in `state/runtime_state.py`. Importing `AnalystDecision` into `runtime_state.py` would create an import dependency from the state layer into the agents layer — the wrong direction. State should be dependency-free so it can be initialized and read without pulling in agent modules. `Dict[str, Any]` keeps the TypedDict clean; the type annotation documents the intent without creating the import.

---

### Step 5.5 — Exhaustion Handling and Graceful Continuation

**What happens:** After `run_loop` returns, the orchestrator checks whether the final result was Critic-approved or whether the loop exhausted all 5 iterations:

```python
decision = run_loop(generator_fn, critic_fn, max_iter=5)

# Check if we exhausted without approval (D-06)
...
# Store and continue regardless
state["analyst_decisions"][column] = decision
state["insights"][column] = _to_insights_bridge(decision)
state["analyzed_columns"].add(column)
```

**Why call `critic_fn` again after `run_loop` returns?**

`run_loop` returns `last_result` in both cases — whether it exited because the Critic approved, or because it hit `max_iter`. It has no return channel to communicate which case occurred. The only way to distinguish is to call `critic_fn` once more on the returned result. Since `validate_finding` is a pure deterministic function (no API calls, no side effects), this extra call costs nothing and produces the information needed to log the right warning.

**Why continue to the next column even when exhausted?**

Exhaustion means the Analyst couldn't produce a finding the Critic could fully verify in 5 tries. Two options:

**Option A (abort column):** Skip the column, log a warning, move on. The column produces no finding.

**Option B (use best attempt):** Use the last `AnalystDecision` produced — even if some claims weren't verified. Log a warning that this finding is lower confidence.

Option A sounds safer but produces a worse report. If a column is high-risk (that's why it was selected), leaving it out of the report means the user misses an important signal. Option B gives the user the finding with an implicit caveat: the Analyst's best attempt after 5 tries. The Critic's `rejected_claims` will have been incorporated into each retry, so the final attempt is typically the most refined. A finding that didn't fully satisfy the Critic is still more useful than no finding at all.

The hard cap of 5 iterations was established in Phase 3. The exhaustion-and-continue pattern was also established in Phase 3 (`run_loop` always returns `last_result`). Phase 5 simply honors that design: `run_loop` is the contract, and the contract says "always return something, never raise".

---

### Step 5.6 — df Boundary Smoke Test

**What the test does:**

```python
def test_run_agent_df_boundary(monkeypatch):
    """No agent module should receive a pd.DataFrame."""
    received_args = {}

    def mock_analyze_column(state, column, rejected_claims=None):
        # Inspect every argument — if df leaked in, it would appear here
        for arg in [state, column, rejected_claims]:
            assert not isinstance(arg, pd.DataFrame), \
                f"DataFrame leaked into analyze_column as {type(arg)}"
        received_args[column] = True
        return _minimal_analyst_decision(column)

    monkeypatch.setattr("orchestrator.orchestrator.analyze_column", mock_analyze_column)
    state = _minimal_state()
    run_agent(state=state, df=pd.DataFrame({"revenue": [1, 2, 3]}), config=CONFIG)
    assert len(received_args) > 0  # prove analyze_column was actually called
```

**Why this test specifically?**

The df boundary is the entire justification for `build_analyst_context()` existing (Phase 4) and `_run_tools_for_column()` being a separate function (Phase 5). If either of those structural decisions were accidentally bypassed — for instance, if a future developer added `df` to `analyze_column`'s signature for convenience — the sentinel test would catch it immediately.

Code review catches intentional choices. This test catches accidental ones: a developer two months from now, unfamiliar with the boundary decision, trying to make the Analyst richer by giving it direct DataFrame access. They add `df` to the parameter list, tests pass (because existing tests don't check for DataFrame in args), they ship, and the privacy guarantee is silently broken.

The smoke test makes the boundary self-enforcing. It will break loudly if anyone violates it, regardless of whether they knew about the decision.

---

### Step 5.7 — Preserving the v2 Output Contract

**What doesn't change in Phase 5:**
...
`run_agent` keeps the same signature: `(state, df, config) → dict`. The return dict keeps the same shape: `{"status": ..., "columns_analyzed": ..., "total_columns": ...}`. `state["insights"]` is populated after every column (via the bridge in Step 5.4). The report generator, plot generator, and LLM report writer all consume the same state keys they always have.

**Why treat backward compatibility as a first-class design constraint, not an afterthought?**

Because Phase 6 is a separate phase. If Phase 5 changed the output format too, then any regression in Phase 5 would break both the orchestrator and the report — two things to debug simultaneously. By keeping the v2 output contract intact, Phase 5 can be tested independently: if `outputs/report.md` still looks correct after Phase 5, the orchestrator wiring is correct. If `outputs/report.md` is broken after Phase 5, you know exactly where to look.

The discipline of "one phase, one responsibility" is what makes the six-phase plan executable without a test suite that covers every combination. Each phase produces a stable state of the system. Each subsequent phase builds on a verified foundation.

---

### Phase 5 Tests (TDD)

Same Wave 0 (RED) → Wave 1 (GREEN) structure as prior phases.

**Wave 0 — TDD scaffold:** Create `tests/test_orchestrator.py` with three stub tests that fail with `NotImplementedError`.

**Wave 1 — Implementation:** Rewrite `run_agent()`, extend `AgentState`, implement `_run_tools_for_column` and `_make_gate1_critic`. Convert stubs to GREEN.

Key tests:

| Test | What it proves |
|------|---------------|
| `test_run_agent_df_boundary` | `analyze_column` never receives a `pd.DataFrame` |
| `test_gate1_exhaustion_logs_warning` | When Critic never approves, `logging.warning` fires and `run_agent` returns normally |
| `test_run_agent_integration_smoke` | End-to-end call with mocked Analyst and Critic populates `analyst_decisions`, `insights`, and `analyzed_columns` correctly |

Plus the full regression suite: all 48 prior-phase tests (state schema, temporal profiler, Critic, Ralph Loop, LLM Analyst) must remain GREEN after the orchestrator is rewired.
